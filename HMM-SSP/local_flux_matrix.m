...
function W = local_flux_matrix(vertex_loc,area,xk,cg);
...
% Matrix D_K=diag(|sigma|/d_Ksigma n_Ks.n_Ks).
%		d_Ksigma=(xs-xk).n_Ksigma.
% 		diag(N*N') is "(n_Ks.nKs)_{s edge of K}"
D=diag(msigma./diag(X*N').*diag(N*N'));
